import { add, roundShekels } from '@/lib/tax/decimal'
...
export function mapToFields(result: TaxResult): MappedField[] {
  ...
  return [
    ...
    { form: '1301', code: f('dividend25').code, status: f('dividend25').status, field: 'foreign-dividends', labelKey: 'field.foreignDividends', valueIls: roundShekels(dividendsGross) },
    ...
    { form: '1301', code: f('foreignIncomeTotal').code, status: f('foreignIncomeTotal').status, field: 'foreign-income-total', labelKey: 'field.foreignIncomeTotal', valueIls: roundShekels(add(dividendsGross, interestGross)) },
    { form: '1324', code: f('ftcDividendTax').code, status: f('ftcDividendTax').status, field: 'foreign-tax-credit', labelKey: 'field.foreignTaxCredit', valueIls: roundShekels(result.totalCreditIls) },
